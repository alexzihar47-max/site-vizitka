...
(function () {
  ...
  const ELONG = 1.12; // вытянута вперёд-назад

  // Край каски поднимается от козырька к бокам и чуть опускается сзади.
  function rimRise(u) {
    const s = Math.sin(u);
    return 0.09 * (1 - Math.max(0, s)) - 0.035 * Math.pow(Math.max(0, -s), 2);
  }
  ...
  function domePoint(u, v) {
    ...
    const ax = Math.abs(x);
    const relief = 0.045 * (1 - smoothstep(0.11, 0.21, ax)) +
      0.028 * Math.exp(-Math.pow((ax - 0.44) / 0.065, 2));
  ...
  function brimPoint(u, t, bottom) {
    ...
    const r = 1 + (outer - 1) * t;
    let y = 0.01 + rimRise(u) - 0.05 * Math.pow(t, 1.5) - 0.12 * t * front * front;
    if (bottom) y -= 0.045;
  ...
  addSurface(function (a, b) {
    const top = brimPoint(TAU_A(a), 1, false);
    const bottom = brimPoint(TAU_A(a), 1, true);
    return [top[0], top[1] + (bottom[1] - top[1]) * b, top[2]];
  }, SEG, 1, function (p) { return [p[0], 0, p[2]]; }, WHITE);

  // чёрное оголовье внутри: лента, видна снизу по бокам и сзади
  function bandPoint(u, b, r) {
    return [r * Math.cos(u), 0.08 - 0.34 * b, r * Math.sin(u) * ELONG * 0.96];
  }
  ...
  const KNOB = [0, -0.17, -0.86 * ELONG * 0.96];
  addSurface(function (a, b) {
    const u = TAU_A(a);
  ...
  [0.55, -0.55, Math.PI + 0.55, Math.PI - 0.55].forEach(function (u) {
    const base = domePoint(u, HALF * 0.86);
    const out = norm([Math.cos(u), 0, Math.sin(u) * ELONG]);
    const along = norm([-Math.sin(u) * ELONG, 0, Math.cos(u)]);
    addBox(add(base, scale(out, 0.02)), along, [0, 1, 0], out, 0.1, 0.07, 0.04, WHITE);
  });
  ...
  if (gl) {
    const vs = [
      ...
      '  vObj = aPos;',
      '  vCol = aCol;',
      '  vec3 p = uRot * (aPos - vec3(0.0, 0.34, 0.0));',
      '  vN = uRot * aNor;',
      '  vP = p;',
      '  float w = uCam - p.z;',
      '  gl_Position = vec4(p.x * uF, p.y * uF + 0.06 * w, -p.z / 3.0 * w, w);',
      '}'
    ].join('\n');
    ...
    if (gl.getProgramParameter(prog, gl.LINK_STATUS)) {
      ...
      gl.uniform1f(gl.getUniformLocation(prog, 'uF'), 2.8);
  ...
  function frame(now) {
    ...
    if (draw) {
      if (!reduceMotion) {
        draw(yawAt(now), 0.3 + 0.03 * Math.sin(now / 900));
      } else if (!drewStatic) {
        draw(0.45, 0.3);
        drewStatic = true;
      }
